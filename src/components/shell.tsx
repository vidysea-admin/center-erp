        ...
        <div className="relative">
          ...
          {userMenu && (
            <div className="absolute right-0 top-11 z-50 w-48 rounded-xl border bg-white py-1.5 shadow-lg">
              ...
              {/* QA-1829a: every role owns their own password, so this is not gated. It is the only
                  way a person can change it - PATCH /api/users/[id] refuses a self-edit on
                  credentials, correctly, and always did. */}
              <button onClick={() => { setUserMenu(false); router.push("/change-password"); }}
                className="block w-full px-4 py-2 text-left text-sm hover:bg-gray-50">Change password</button>
