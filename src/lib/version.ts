...
const RELEASE_NOTE_ARCHIVE_310 =
  ...
  "proposed. Leaving that choice untouched creates the head as proposed, and where a head "  +
  "was chosen the decision records which one. Whether a cost can reach that state depends "  +
  "on your administrator having switched on approval for new cost heads; with "  +
  "that switch off, an administrator naming a new head creates it as they go and everybody "  +
  "else is asked to pick an existing head instead. "  +
...
const RELEASE_NOTE_ARCHIVE_309 =
  ...
  "One related change, on Finance, on Revenue and P&L, and on a trainer's own page: a "  +
  "job role that an administrator has switched off is no longer offered where a job role "  +
  "is chosen. It was being listed whether it was "  +
...
const RELEASE_NOTE_ARCHIVE_302 =
  ...
  "editable, and moves to Result Awaited only after the assessment is explicitly recorded as held. "  +
  "The batch register and reporting use the same two stage labels. This release also completes the "  +
  "cost workflow requested for centres: authorised operational staff can submit scoped costs while "  +
  "the finance ledger remains restricted; formula-based assessment fees are capped by billable "  +
  "passed candidates; an approver can approve a lower amount with a recorded reason; and payment "  +
  "date, reference and payee reach the outgoing-payment report. The person who raised a request "  +
  "still cannot approve it. Finally, staff invitation buttons now carry and lock the intended email, "  +
  "sign out any different account already open in that browser, and require the invited person's "  +
  "password. This closes the case where opening Karunn's mail while an Admin session was active "  +
  "appeared to sign him in without a password; no password was present in the mail or URL.";
const RELEASE_NOTE_ARCHIVE_301 =
...
const RELEASE_NOTE_ARCHIVE_294 =
  ...
  "its own arithmetic spelled out. Nothing is written by itself - a person still raises the " +
  "invoice. Against a raised invoice you can now record what was actually received, with a " +
  "reference, and receiving less than was invoiced is allowed and shown as a shortfall rather " +
  "than quietly rounded away, because a part payment or a tax deduction is the exact thing a " +
  "settlement report exists to trace. A new profit-and-loss view puts accrued, invoiced, " +
  "received, cost and margin side by side per batch and rolls them up by centre, job role and " +
  "scheme, and every row says which stage of the chain it has reached. Where a scheme has no " +
  "rate recorded the earning is left blank with the reason named, never printed as zero - a " +
  "missing rate and a rate of nothing are different facts - and margin is withheld entirely " +
  "rather than published on top of an unknown. A cost entry now records what a voucher records: " +
  "who was paid, against which voucher number, by which payment mode, and a description that is " +
  "required rather than optional. Both places a cost can be entered ask for the same things. The " +
  "cost head list is shown grouped as head and sub-head instead of one flat list, and somebody " +
  "who cannot find the right head can name the one they need: the whole entry then waits for an " +
...
const RELEASE_NOTE_ARCHIVE_291 =
  ...
  "one to confirm before picking, on the same preview screen - the phone is never used as an " +
  "automatic match, only shown for the human to read.";
const RELEASE_NOTE_ARCHIVE_290 =
